...
public class Sort {
    ...
    /**
     * 搜索旋转排序数组（首先寻找第一个下降点O(log n),然后在左侧或者右侧进行二分查找，时间复杂度打败100%，内存消耗比较大）
     */
    static class Search {
        public int search(int[] nums, int target) {
            int declinePoint = binarySearchDeclinePoint(nums, 0, nums.length-1);
            //如果根本没有下降点
            if(declinePoint == -1){
                return binarySearch(nums, 0, nums.length-1, target);
            }else{
                if(target >= nums[0] && target <= nums[declinePoint])
                    return binarySearch(nums, 0, declinePoint, target);
                else if (target >= nums[declinePoint+1] && target <= nums[nums.length-1])
                    return binarySearch(nums, declinePoint + 1, nums.length-1, target);
                else
                    return -1;
            }
        }

        public int binarySearchDeclinePoint(int[] nums, int begin, int end){
            ...
            //不是下降点，则下降点在其左边或者右边
                ...
                int left = binarySearchDeclinePoint(nums, begin, mid-1);
                int right = binarySearchDeclinePoint(nums, mid+1, end);
        ...
        /**
         * 答案解法（更巧妙一些，其思路主要是target左右肯定有一个是有序的，我们在有序的部分进行查找，找不到了再在无序部分查找）
         */
    ...
    /**
     * 搜索二维矩阵 II（我的解法一，做n次二分搜索，可以，但是效率太低）
     */
    static class SearchMatrix {
        ...
        /**
         * 我的解法二：首先排除那些肯定不在的单元格子（性能提升还是非常大的）
         */
        public boolean searchMatrix(int[][] matrix, int target) {
            int start = 0, end = Math.min(matrix.length, matrix[0].length) - 1;
            int mid = -1;
            ...
            while (start <= end){
                ...
                if(mid+1 > end || matrix[mid][mid] <= target && matrix[mid+1][mid+1] > target)
                    break;
            ...
            for(int i = mid;i < matrix.length;i++){
                start = 0;
            ...
            for(int i = mid;i < matrix[0].length;i++){
                start = 0;
                end = matrix.length - 1;
                while (start <= end){
                    mid = start + (end - start) / 2;
        ...
        /**
         * 答案解法(标准答案，从第一行最后一个元素开始对比，如果大了，就往左移动，如果小了就往下移动，执行用时更快，内存消耗比我的方法二大)
         */
...
//        new SearchMatrix().searchMatrix(new int[][]{{-5}}, -5);
        ...
        new SearchMatrix().searchMatrix(new int[][]{{1,3,5}}, 2);
    }
}
